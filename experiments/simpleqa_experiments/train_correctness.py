#!/usr/bin/env python
"""
train_correctness.py
====================
Train the SimpleQA correctness probe (Table 2, in-domain row) and evaluate it
on a held-out set, reporting accuracy, balanced (macro) accuracy, AUROC, Brier,
and ECE with bootstrap CIs, then save the probe.

The probe is trained on a fixed number of examples per class (--train_class_sizes),
and everything NOT selected for training is held out for evaluation. For the
SimpleQA correctness experiment the training draw is 300 incorrect (class 0) and
200 correct (class 1); the held-out set is therefore the remainder of the scored
questions and RETAINS SimpleQA's natural class imbalance (see run_simpleqa_eval.sh).
Because that held-out set is heavily imbalanced, AUROC (not raw accuracy) is the
metric of record; raw accuracy is reported alongside the majority-class baseline.
----------------------------------------------------------------------------
Example (reproduces the Table 2 SimpleQA in-domain row; see run_simpleqa_eval.sh)
-------
$ python train_correctness.py \
      --data_dirs       /path/to/simpleqa_embeddings \
