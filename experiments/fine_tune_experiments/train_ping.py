...
parser.add_argument("--model", type=str, default="mlp", help="Model type (e.g., 'mlp')")
...
y_train = pd.read_csv(train_response_df, index_col="id")["gold_answer_norm"]
...
y_test = pd.read_csv(test_response_df, index_col="id")["gold_answer_norm"]
...
# NEW: Multiclass ROC AUC (macro-averaged, One-vs-Rest)
classes     = np.unique(y_test.values)
auc_point   = float(roc_auc_score(y_test.values, probs, multi_class="ovr", average="macro", labels=classes))
...
# classes already computed above from y_test
...
auc_samples   = np.empty(B_BOOTSTRAP, dtype=np.float64)  # NEW
...
for b in range(B_BOOTSTRAP):
    ...
    pb        = pred[idx_boot]
    cb        = conf[idx_boot]
    probs_b   = probs[idx_boot]  # NEW: full probability matrix for AUC
    ...
    # NEW: macro OvR AUC on multiclass probabilities
    auc_samples[b]   = roc_auc_score(yb, probs_b, multi_class="ovr", average="macro", labels=classes)
...
auc_ci   = percentile_ci(auc_samples,   alpha=0.05)  # NEW
...
- ROC AUC (macro OvR):  {auc_point:.4f}
...
- ROC AUC CI:           [{auc_ci[0]:.4f}, {auc_ci[1]:.4f}]
...
metrics_json = {
    ...
    "point": {
        "accuracy": acc_point,
        "auc_macro_ovr": auc_point,  # NEW
        "brier": brier_point,
        "ece": ece_point
    },
    "ci_95": {
        "accuracy": {"low": acc_ci[0], "high": acc_ci[1]},
        "auc_macro_ovr": {"low": auc_ci[0], "high": auc_ci[1]},  # NEW
...
print(
    f"ACC {acc_point:.4f}  AUC(macro OvR) {auc_point:.4f}  "
    f"Brier {brier_point:.4f}  Binary-ECE {ece_point:.4f} | "
